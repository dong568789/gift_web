...
<form action="<{'admin'|url nofilter}>/<{block "name"}><{/block}>/" method="GET" class="form-bordered form-horizontal">
	...
	<div class="form-group col-sm-4">
		<label class="col-md-3 control-label" for="send_account">发货</label>
		<div class="col-md-9">
			<div class="input-group">
				<select name="f[send_account]" id="send_account" class="form-control">
					<option value="">请选择</option>
					<{foreach catalog_search('status.send_account', 'children') as $item}>
				<option value="<{$item.id}>" <{if $_filters.send_account.eq == $item.id}>selected<{/if}>><{$item.title}></option>
					<{/foreach}>
				</select>
			</div>
		</div>
